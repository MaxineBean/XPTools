	...
	for i=1, #target_plys do
		local v = target_plys[ i ]
			calling_ply:ChatPrint('VIEWING STATS OF ' .. v:Nick() .. ' (' .. v:SteamID() .. ')')
		 	calling_ply:ChatPrint('Level: ' .. v.Maxwelllvl)
		 	calling_ply:ChatPrint('XP: ' .. v.Maxwellxp)
			table.insert( affected_plys, v )
	end
...
function ulx.getstats( calling_ply )
	...
	for k,v in pairs(player.GetAll()) do
		...
		calling_ply:PrintMessage(HUD_PRINTCONSOLE, 'Level: ' .. v.Maxwelllvl)
		calling_ply:PrintMessage(HUD_PRINTCONSOLE, 'XP: ' .. v.Maxwellxp)
	end
...
function ulx.viewlevel( calling_ply, target_plys )
	...
	for i=1, #target_plys do
		local v = target_plys[ i ]
		 	calling_ply:ChatPrint(v:Nick() .. ' is level ' .. v.Maxwelllvl)
			table.insert( affected_plys, v )
	end
